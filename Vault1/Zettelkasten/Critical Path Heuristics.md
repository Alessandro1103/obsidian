...
The second and so on, the size of the goal, in this case $|g| \leq m = 2 \leq 1$ is false, so we take the $max$ option. So we have to cut the goal sets, until we get $|g| = m = 1$. For example, we have $G:\{t(A), p(D)\}$, so we get to  
