...
- $min [h_1^1(t(B),t(A)), min[ c(a) + h_1^1(s, regr(t(A), a))]]$
but $h^1_1$ is unknown so we get back to the first definition $h_0^m$ and we get $\infty$, so at the end the MIN is 1 came from c(a). We can do the same for $p(C)$, 
Example m=2:
